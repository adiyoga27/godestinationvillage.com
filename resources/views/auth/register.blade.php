...
<section class="relative min-h-screen bg-ink-950">
    ...
    <div class="container-gd relative z-10 flex items-center justify-center py-20">
        <div class="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-2xl sm:p-10">
            ...
            <h1 class="mt-6 text-center font-display text-2xl font-bold text-ink-950">Create your account</h1>
            <p class="mt-1 text-center text-sm text-ink-500">Join GODEVI to book village experiences and manage your reservations.</p>
            ...
            <form method="POST" action="{{ route('register') }}" class="mt-7 space-y-5">
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <label class="block">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="Your name" class="input-gd">
                    </label>
                    <label class="block">
                ...
                <label class="flex items-start gap-2 text-sm text-ink-600">
                    ...
                    <span>I agree to the <a href="{{ url('term') }}" class="font-bold text-brand-600 hover:underline">Terms of Service</a>.</span>
                </label>
                <button type="submit" class="btn btn-primary w-full !py-3.5">Create Account</button>
            </form>
