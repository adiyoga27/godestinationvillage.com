...
<x-partials.page-hero
    title="{{ __('Paid Reservation') }}"
    subtitle="{{ __('Your confirmed homestay reservations.') }}"
    image="assets/customer/img/page-title-area/privacy.jpg"
    :crumbs="[__('Home') => '/', __('Reservation') => '', __('Paid') => '']"
/>
...
<section class="section-pad bg-cream-50">
    <div class="container-gd">
        <div class="grid gap-8 lg:grid-cols-3">
                ...
                <div class="space-y-6">
                    ...
                    @empty
                        <div class="card p-12 text-center">
                            <p class="font-display text-xl font-bold text-ink-950">{{ __('No paid reservations') }}</p>
                            <p class="mt-2 text-sm text-ink-500">{{ __('You have no confirmed homestay bookings yet.') }}</p>
                        </div>
                    @endforelse
                </div>
                @if (count($order) > 0)
            ...
            <div>
                <x-partials.reservation-sidebar :email="$isiemail" base="reservation-homestay" active="paid" title="{{ __('Status Booking Homestay') }}" />
            </div>
        </div>
    </div>
</section>
