...
<x-partials.page-hero
    title="Booking Homestay — Tanpa Login"
    subtitle="Alur 4 — {{ $packages->name }}. Pembayaran via Midtrans Snap."
    image="assets/customer/img/page-title-area/account.jpg"
    :crumbs="['Home' => '/', 'Homestay' => '/homestay', 'Guest Booking' => '']"
/>
...
<section class="section-pad bg-cream-50">
    <div class="container-gd max-w-4xl">
        ...
        <form action="{{ route('guest-booking.homestay.store') }}" method="post" class="space-y-8">
            @csrf
            <input type="hidden" name="idhomestay" value="{{ $packages->id }}">

            <div class="card p-8 sm:p-10">
                <h2 class="font-display text-xl font-bold text-ink-950">Homestay: {{ $packages->name }}</h2>
                <p class="mt-2 text-sm text-ink-500">Harga/malam: Rp {{ number_format($packages->disc > 0 ? $packages->disc : $packages->price, 0, ',', '.') }}</p>
            </div>
            ...
            <div class="card p-8 sm:p-10">
                ...
                <div class="mt-6 grid gap-5 sm:grid-cols-2">
                    ...
                    <label class="block"><span class="label-gd">Jumlah Tamu *</span><input type="number" name="pax" min="1" value="{{ old('pax', 1) }}" required class="input-gd"></label>
                    <label class="block"><span class="label-gd">Tanggal Check-in</span><input type="date" name="checkin_date" value="{{ old('checkin_date') }}" class="input-gd"></label>
            ...
            <div class="card flex flex-col gap-4 p-8 sm:flex-row sm:items-center sm:justify-between sm:p-10">
                <p class="text-sm text-ink-500">Lanjut ke pembayaran Midtrans (Snap embed).</p>
