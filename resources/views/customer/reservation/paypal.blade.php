...
@php
    $seo = \App\Support\Seo::make()->title('PayPal Reservations — GODEVI')->noindex()->toArray();
@endphp

<x-partials.page-hero
    title="{{ __('PayPal Reservations') }}"
    subtitle="{{ __('Review reservations paid through PayPal.') }}"
    image="assets/customer/img/page-title-area/privacy.jpg"
    :crumbs="[__('Home') => '/', __('Reservation') => '', __('PayPal') => '']"
/>
                ...
                <div class="space-y-6">
                    ...
                    @empty
                        <div class="card p-12 text-center">
                            <p class="font-display text-xl font-bold text-ink-950">{{ __('No PayPal transactions') }}</p>
                            <p class="mt-2 text-sm text-ink-500">{{ __('Your PayPal reservations will appear here.') }}</p>
                        </div>
                    @endforelse
                </div>
                @if (count($order) > 0)
