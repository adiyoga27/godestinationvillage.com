...
<x-partials.page-hero
    ...
    subtitle="{{ __('Complete your bank transfer confirmation so our team can verify and approve your booking.') }}"
    image="assets/customer/img/page-title-area/privacy.jpg"
    :crumbs="[__('Home') => '/', __('Confirm Payment') => '']"
/>
    ...
    <div class="container-gd mx-auto max-w-2xl">
        <div class="card overflow-hidden">
            ...
            <form action="{{ url('payment/pay/confirm-payment') }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-8 sm:p-10">
                ...
                <div>
                    <label for="bank" class="mb-2 block text-sm font-bold text-ink-800">{{ __('Your Bank Account') }}</label>
                ...
                <div>
                    <label for="name" class="mb-2 block text-sm font-bold text-ink-800">{{ __('Name Account') }}</label>
                    <input type="text" name="name" id="name" placeholder="{{ __('Enter your bank account name') }}"
                ...
                <div>
                    <label for="date" class="mb-2 block text-sm font-bold text-ink-800">{{ __('Date of Transfer') }}</label>
                    <input type="text" name="date" id="date" placeholder="22/04/2019"
                        class="w-full rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm text-ink-800 placeholder-ink-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                </div>

                <div>
                    <label for="message" class="mb-2 block text-sm font-bold text-ink-800">{{ __('Message') }}</label>
                    <input type="text" name="message" id="message" placeholder="{{ __('Message') }}"
                        class="w-full rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm text-ink-800 placeholder-ink-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                </div>
