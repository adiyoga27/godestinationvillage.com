...
<x-partials.page-hero
    ...
    subtitle="{{ $keyword ? 'Packages matching "' . $keyword . '"' : 'Enter a keyword to find the perfect village experience.' }}"
...
<section class="section-pad">
    <div class="container-gd">
        <form action="{{ url('search') }}" method="GET" class="mx-auto mb-12 max-w-2xl">
            <div class="relative">
                <input type="text" name="key" value="{{ $keyword }}" placeholder="Search tour packages..."
                ...
                <button type="submit" class="btn btn-primary absolute right-2 top-1/2 -translate-y-1/2 !py-2.5 !text-sm">Search</button>
            </div>
        </form>

        @if (!$keyword)
            <p class="text-center text-ink-500">Type a keyword above to search our tour packages.</p>
        @else
            @if (count($packages) > 0)
                <p class="mb-8 text-sm text-ink-500"><strong class="text-ink-900">{{ $packages->total() }}</strong> package(s) found for "<strong class="text-brand-600">{{ $keyword }}</strong>"</p>
            @endif
                    ...
                    <a href="{{ url('tour-packages/' . $pack->slug) }}" data-vue="Reveal" class="group card card-hover flex flex-col overflow-hidden">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <div class="mt-5 flex items-center justify-between border-t border-ink-50 pt-4">
                                ...
                                <span class="text-xs font-bold uppercase tracking-wide text-brand-600 transition group-hover:translate-x-1">View →</span>
                            </div>
                        </div>
                    </a>
                @empty
                    <div class="card col-span-full p-12 text-center">
                        <p class="font-display text-xl font-bold text-ink-950">No results found</p>
                        <p class="mt-2 text-sm text-ink-500">We couldn't find any packages matching "{{ $keyword }}". Try a different keyword.</p>
                        <a href="{{ url('tour-packages') }}" class="btn btn-primary mt-6">Browse All Packages</a>
                    </div>
                @endforelse
            </div>
        @endif
        ...
        @if ($keyword && $packages->hasPages())
            <div class="mt-10 flex items-center justify-center gap-2">
                ...
                @if ($packages->lastPage() > 0 && $packages->currentPage() < $packages->lastPage())
                    <a href="{{ $packages->nextPageUrl() }}" class="btn btn-secondary !px-5 !py-2.5">Next</a>
                @endif
            </div>
        @endif
    </div>
