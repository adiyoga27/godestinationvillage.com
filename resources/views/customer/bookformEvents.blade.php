...
<x-partials.page-hero
    ...
    :crumbs="[__('Home') => '/', __('Events') => '/events', __('Reservation') => '']"
/>
    ...
    <div class="container-gd max-w-4xl">
        <form action="{{ $packages->is_free ? url('bookingEvents/sendEventFree') : url('bookingEvents/sendEvent') }}" method="post" class="space-y-8">
            ...
            <div class="card p-8 sm:p-10">
                <h2 class="font-display text-xl font-bold text-ink-950">{{ __('Event Information') }}</h2>
                <div class="mt-6 grid gap-5 sm:grid-cols-2">
                    <div>
                        <span class="label-gd">{{ __('Event Name') }}</span>
            ...
            <div class="card flex flex-col gap-6 p-8 sm:flex-row sm:items-center sm:justify-between sm:p-10">
                <div>
                    ...
                    @if ($packages->is_free)
                        <p class="mt-1 font-display text-3xl font-bold text-forest-600">{{ __('Gratis') }}</p>
                    @else
