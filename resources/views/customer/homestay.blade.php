...
<x-partials.page-hero
    ...
    subtitle="Wake up to village life — stay with local families and experience genuine Balinese hospitality."
    image="assets/customer/frontdata/images/bg_1.jpg"
    :crumbs="['Home' => '/', 'Homestay' => '']"
/>
        ...
        <div class="grid gap-7 md:grid-cols-2 lg:grid-cols-3">
                ...
                <a href="{{ url('homestay/' . $pack->id) }}" data-vue="Reveal"
                    ...
                    <div class="relative h-56 overflow-hidden">
                        <img src="{{ $pack->default_img ? asset('storage/homestay/' . $pack->default_img) : asset('assets/customer/frontdata/images/destination-' . (($loop->index % 6) + 1) . '.jpg') }}"
                            alt="{{ $pack->name }} — homestay desa di Bali" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy">
                    ...
                    <div class="flex flex-1 flex-col p-6">
                        <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-ink-400">
                            ...
                            @if ($pack->is_breakfast)
                                <span class="badge bg-cream-50 text-forest-700">Breakfast included</span>
                            @endif
                        </div>
                        <h2 class="mt-2 font-display text-xl font-semibold text-ink-950 transition group-hover:text-brand-600">{{ $pack->name }}</h2>
                        <p class="mt-2 line-clamp-2 flex-1 text-sm text-ink-500">{{ strip_tags($pack->description ?? '') }}</p>
                        @if ($pack->owner_name ?? null)
                            <p class="mt-3 text-xs font-semibold text-ink-400">Hosted by {{ $pack->owner_name }}</p>
                        @endif
                        <div class="mt-5 flex items-center justify-between border-t border-ink-50 pt-4">
                            <span class="text-lg font-bold text-brand-600">Rp {{ number_format($pack->price, 0, ',', '.') }}/night</span>
            ...
            @empty
                <div class="col-span-full py-20 text-center">
                    <p class="text-ink-400">No homestays available yet.</p>
                </div>
            @endforelse
        </div>
